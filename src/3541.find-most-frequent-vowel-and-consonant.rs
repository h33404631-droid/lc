...
// @lc app=leetcode id=3541 lang=rust
//
...
impl Solution {
    pub fn max_freq_sum(s: String) -> i32 {
        ...
        let vowels: std::collections::HashSet<_> = "aeiou".chars().collect();
        let max_vowel = (0..26)
            .filter(|&i| vowels.contains(&((i as u8 + b'a') as char)))
            .map(|i| chars[i])
            .max()
            .unwrap_or(0);
        let max_consonant = (0..26)
            .filter(|&i| !vowels.contains(&((i as u8 + b'a') as char)))
            .map(|i| chars[i])
            .max()
            .unwrap_or(0);
